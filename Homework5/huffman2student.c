...
void insertSortLink(struct node *n,int mode){
    if(Head == NULL){
        Head = n;
    }else{
        struct node *p = NULL;
        struct node *q = NULL;
        if(mode){
            for(p=Head; p != NULL && n->elem->weight >= p->elem->weight ; q = p, p = p->link);
        }else{
            for(p=Head; p != NULL && (n->elem->weight > p->elem->weight ||
                                      (n->elem->weight == p->elem->weight && n->elem->c > p->elem->c)); q = p, p = p->link);
        }
        if(p==Head){
            n->link = Head;
            Head = n;
...
void createHTree()
{
    ...
    for(int i=0; i<128; i++){
        if(Ccount[i]>0){
            ...
            temp->elem = p;
            temp->link = NULL;
            insertSortLink(temp,0);
        }
    }
    while(Head->link != NULL){
        p = (struct tnode *)malloc(sizeof(struct tnode));
        p->weight = Head->elem->weight + Head->link->elem->weight;
        p->left = Head->elem; p->right = Head->link->elem;  /*将新树的根结点加入到有序结点链表中*/
        Head = Head->link->link;
        struct node *temp = (struct node *)malloc(sizeof(struct node));
        temp->elem = p;
        temp->link = NULL;
        insertSortLink(temp,1);
    }
    Root = Head->elem;
}
//【实验步骤2】结束
